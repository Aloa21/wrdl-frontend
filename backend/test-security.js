...
async function runTests() {
  ...
  try {
    const res = await fetch(`${API_BASE}/api/game/guess`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ sessionId, guess: 'MONAD' })
    });
  ...
  try {
    const res = await fetch(`${API_BASE}/api/game/guess`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ sessionId, guess: 'MONAD', token: 'wrongtoken' })
    });
  ...
  const WORDS = ['MONAD', 'BLOCK', 'CHAIN', 'TOKEN', 'STAKE', 'CRAFT', 'SMART', 'PROOF', 'VALID', 'NODES'];
  let won = false;
  let winningWord = '';
  let guessCount = 0;
